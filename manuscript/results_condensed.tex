...
Figure~\ref{fig:physical_response} summarizes the resolved thermal and
hydraulic response over the 60-condition matrix.
...
\input{generated_steady_final_windows}}{}
...
\caption{External comparisons: (a) PREMUX thermocouple temperatures,
(b) TESOMEX radial profiles, (c) HELOKA Nusselt numbers and (d) pressure
...
Figure~\ref{fig:external_evidence} compares independent thermal and hydraulic
measurements with the calculations. PREMUX and TESOMEX expose temperature
structure missed by reduced one- and two-dimensional descriptions. HELOKA and
...
seed202; the dashed line denotes parity, colours encode inlet temperature and
...
Figure~\ref{fig:steady_model_comparison} compares all steady models on the
same complete-condition splits and physical quantities.
...
\input{generated_steady_performance}}{}
\IfFileExists{generated_steady_result_text.tex}{%
\input{generated_steady_result_text}}{}
}{}
...
\input{generated_regional_dynamics_text}}{}
}

\IfFileExists{generated_transient_model_comparison_validated.tex}{%
\IfFileExists{../figures/hccb_p418_transient_model_comparison.pdf}{%
Figure~\ref{fig:transient_model_comparison} summarizes the resulting accuracy,
finite-volume energy consistency and acceleration.
\begin{figure}[!htbp]
\centering
...
and loss-weight selection. (a)--(d) Representative solid-temperature
responses. (e) Temperature error versus finite-volume energy-equation RMSE
...
share a scale. Error colours are clipped at the 99th percentile; annotated
...
\SI{\PFieldSolidMaxError}{K}.  These maxima are reported without colour-scale
