...
DEFAULT_RANKS = (1, 2, 3, 4, 8, 12, 16, 24, 32)
...
def field_metrics(
    ...
    if not np.all(np.isfinite(prediction)) or not np.all(np.isfinite(target)):
        raise FloatingPointError("temperature histories contain non-finite values")
    ...
    for step_size in delta_t:
        ...
        if not np.all(np.isfinite(temperature)):
            raise FloatingPointError(
                "continuous-time DMDc prediction became non-finite"
            )
...
def main() -> int:
            ...
            validation_scan.append(
                {
                    ...
                    "fitted_rank": int(model["rank"]),
                    "stable_on_validation_curves": False,
                    "failure_reason": "validation metrics are non-finite",
                }
            )
            continue
        validation_scan.append(
    ...
    if not stable_candidates:
        raise ValueError("all DMDc rank candidates are unstable on validation curves")
    selected = min(
    ...
    summary = {
        ...
        "unstable_validation_rank_count": len(validation_scan)
