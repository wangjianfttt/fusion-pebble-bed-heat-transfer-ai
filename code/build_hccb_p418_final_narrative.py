...
def build(args: argparse.Namespace) -> dict:
    ...
    abstract = (
        "Pore-scale heat transfer in solid-breeder beds combines "
        ...
        f"{energy_triplet} on the common regional basis. {best_transient_sentence} "
        f"{diffusion_sentence} "
        f"On six high-velocity histories, the "
        f"{HIGH_RE_LABELS[best_high_re_model]} gives fluid/solid RMSEs of "
        f"{fmt(best_high_re_fluid)} and {fmt(best_high_re_solid)} K. Across nine matched conditions on an "
    ...
    discussion = (
        f"{dominant_labels[dominant_effects['pressure_drop_Pa']]} and "
        f"{dominant_labels[dominant_effects['solid_maximum_temperature_K']]} dominate pressure drop and maximum solid temperature, respectively; "
        f"wall heat reverses at {fmt(crossing_min)}--{fmt(crossing_max)} K.\n\n"
        "Physical terms do not guarantee lower temperature error. On the strict split, "
        f"the graph models give temperature RMSEs of {transient_triplet} K and projection-aware energy RMSEs of {energy_triplet}. "
        f"{best_transient_sentence} "
        + (
            "No learned field model outperforms this zero-training baseline on the small velocity and heat-source responses. "
            if persistence_leads
            else ""
        )
        + cost_sentence
        + f"Validation selected {loss_balancing['label']}; its test gave a solid-temperature RMSE of "
        f"{fmt(float(loss_balancing['test_solid_temperature_RMSE_K']))} K and energy nRMSE of "
        f"{fmt(float(loss_balancing['test_projection_aware_energy_normalized_RMSE']))}. "
        f"Diffusion changes temperature RMSE from {fmt(deterministic_temperature)} to {fmt(refined_temperature)} K "
        f"and energy RMSE from {fmt(deterministic_energy)} to {fmt(refined_energy)}. "
        f"{diffusion_sentence} {diffusion_uncertainty_sentence} Maximum seed variation is "
        f"{fmt(float(robustness['maximum_steady_seed_cv_percent']))}\\% for steady nRMSE and "
        f"{fmt(float(robustness['maximum_transient_seed_std_K']))} K for transient temperature. "
        f"For the {STEADY_LABELS[steady_model]}, 9--36 steady conditions change test nRMSE from "
        f"{fmt(float(robustness['steady_learning_low_count_nrmse']))} to "
        f"{fmt(float(robustness['steady_learning_high_count_nrmse']))}; three to six transient curves change RMSE from "
        f"{fmt(float(robustness['transient_three_curve_RMSE_range_K'][0]))}--"
        f"{fmt(float(robustness['transient_three_curve_RMSE_range_K'][1]))} to "
        f"{fmt(float(robustness['transient_six_curve_RMSE_K']))} K.\n\n"
        f"Seed202 preserves integral temperatures but changes pressure drop by {fmt(packing_pressure)}\\%. "
        "HELOKA, fixed-bed, PREMUX and TESOMEX comparisons are aggregate consistency checks, not cellwise validation. "
        "Packing information is necessary because bulk porosity alone did not capture the pressure-drop difference.\n\n"
        "Claims cover a local static pore-resolved domain with prescribed hydrodynamics. "
        "The full-domain fluid mesh failed its basic check, so no full-domain solver was run; fully coupled startup left the specified helium-property range. "
        "These failures do not establish GCI or accuracy. Moving pebbles and blanket manifolds remain outside scope."
    )
    ...
    conclusion = (
        ...
        +
        f"{diffusion_sentence} A second packing changes outlet/maximum-solid temperatures by at most "
        f"{fmt(packing_tout)}/{fmt(packing_tmax)}\\% but pressure drop by {fmt(packing_pressure)}\\%. "
        "The benchmark supports rapid thermal-field prediction within the sampled operating conditions and two packing realizations; "
        "fully coupled startup and blanket-scale prediction remain outside scope."
    )
